...
const ComicCard: React.FC<ComicCardProps> = ({ comic }) => {
  ...
  useEffect(() => {
    const checkMobile = () => {
      setIsMobile(window.innerWidth < 1024); // Consider tablet and below as mobile
    };
  ...
  return (
    <div
      className={`group relative transition-all duration-500 cursor-pointer bg-morphic-gray/60 backdrop-blur-xl rounded-2xl overflow-hidden border border-white/10 
        ${
          isMobile
            ? "w-full max-w-[180px] sm:max-w-[200px] h-[270px] sm:h-[300px]"
            : "flex hover:border-white/20 hover:bg-morphic-gray/80"
        }`}
      style={
        isMobile
          ? {}
          : {
              width: hovered ? 440 : 200,
              minHeight: 300,
              boxShadow: hovered
    ...
    >
      ...
      >
        <img
          ...
          alt={comic.title}
          style={{
            transition: !isMobile ? "transform 0.5s" : "none",
            ...(hovered && !isMobile ? { transform: "scale(1.05)" } : {}),
          }}
        />
        {/* Gradient Overlay */}
      ...
      {/* Expanded Content (only visible on hover and not mobile) */}
      {!isMobile && (
        <div
