...
function parseStoryIntoChapters(storyText: string): Chapter[] {
    ...
    while ((match = chapterPattern.exec(storyText)) !== null) {
        ...
        const readingTime = Math.max(STORY_CONFIG.DEFAULT_READING_TIME, Math.round((wordCount / STORY_CONFIG.WORDS_PER_MINUTE) * 60));
    ...
    if (chapters.length !== STORY_CONFIG.CHAPTERS_COUNT) {
        console.warn(`Expected ${STORY_CONFIG.CHAPTERS_COUNT} chapters but got ${chapters.length}`);
    }

    return chapters;
}

function generateStoryData(storyData: StoryCreate, chapters: Chapter[], generatedStory: string): StoryRead {
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (storyData.description.length > FIELD_CONSTRAINTS.STORY_DESCRIPTION_MAX) {
            return NextResponse.json(
        ...
        try {
            ...
            for await (const chunk of NearAIHelper.generateCompletionStream(chatRequest)) {
            ...
            const errorMessage = nearAIError instanceof Error ? nearAIError.message : "Failed to generate response from Near AI";
            return NextResponse.json(
                { error: errorMessage },
                { status: 500 }
            );
